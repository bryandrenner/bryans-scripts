...
The remaining properties may be supplied as needed, in order and separated by a hyphen.  For example, if a script only applies to Vim 7.x, it should be classifed as `vim-7.x`.  If it only applies to Vim 7.x on Ubuntu, it should be classified as `vim-7.x-ubuntu`. If it only applies to Ubuntu at home, it should be classified as `ubuntu-home`.
